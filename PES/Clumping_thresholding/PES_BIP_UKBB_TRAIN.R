...
write.csv(BIP_results, file="Results/Conservative_C_T_BIP_UKBB_PES_PRS_results.csv", quote = F, row.names = T)
...
write.csv(R2_liability_results, file="Results/Conservative_C_T_BIP_Liability_scale_r2_PES_PRS.csv", row.names = T, quote = F)


## Test covariation of RPS17 PES with PRS at same threshold

RPS17_full <- glm(BIP ~ Sex + Age + PC1 + PC2 + PC3 +
                    PC4 + PC5 + Batch + PRS_0_05 + RPS17_0_05, family = "binomial", data = BIP_merged)
...
GRIN2A_full <- glm(BIP ~ Sex + Age + PC1 + PC2 + PC3 +
                    PC4 + PC5 + Batch + PRS_0_005 + GRIN2A_0_005, family = "binomial", data = BIP_merged)
...
PCCB_full <- glm(BIP ~ Sex + Age + PC1 + PC2 + PC3 +
                     PC4 + PC5 + Batch + PRS_0_05 + PCCB_0_05, family = "binomial", data = BIP_merged)
...
CACNA1C_full <- glm(BIP ~ Sex + Age + PC1 + PC2 + PC3 +
                   PC4 + PC5 + Batch + PRS_0_005 + CACNA1C_0_005, family = "binomial", data = BIP_merged)
...
Cor_BIP <- cor(BIP_merged[,c(3:6, 9:12, 15:18, 21:24, 27:30, 33:36, 39:42)])

pval <- psych::corr.test(BIP_merged[,c(3:6, 9:12, 15:18, 21:24, 27:30, 33:36, 39:42)], adjust = "none")$p
