...
setwd("~/Desktop/SZ_PES_mtCOJO_norm/PES/Replication_scores/UKBB_BIP_validation_replication/")
...
All_BIP_scores <- fread("Combined_PES_all_scores.txt", header = T, sep = "\t")
...
BIP_merged[,c(5, 10, 15, 20, 25)] <- lapply(BIP_merged[,c(5,10, 15, 20, 25)], function(x) c(scale(x)))

## Define function to test each score by itself and extract beta, SE, and P

## List of sets to test

Scores_to_test <- as.list(colnames(BIP_merged[,c(5,10, 15, 20, 25)]))
