...
Scores_to_test <- as.list(colnames(SZ_merged[,c(5, 10, 15, 20, 25, 30, 35)]))
...
write.csv(SZ_results, file="Results/Lassosum_SZ_UKBB_PES_PRS_results.csv", quote = F, row.names = T)
...
write.csv(R2_liability_results, file="Results/Lassosum_SZ_Liability_scale_r2_PES_PRS.csv", row.names = T, quote = F)
...
Cor_SZ <- cor(SZ_merged[,c(5, 10, 15, 20, 25, 30, 35)])

pval <- psych::corr.test(SZ_merged[,c(5, 10, 15, 20, 25, 30, 35)], adjust = "none")$p
...
write.csv(SZ_results, file="Results/BIP_overlap_removed_SZ_UKBB_PES_PRS_results.csv", quote = F, row.names = T)
